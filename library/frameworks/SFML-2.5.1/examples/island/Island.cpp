...
sf::Color getLowlandsTerrainColor(float moisture) {
    sf::Color color =
            ...
            moisture < 0.3f ? sf::Color(240 - 240 * (moisture - 0.27f) / 0.03f, 240 - 40 * (moisture - 0.27f) / 0.03f,
            ...
            moisture < 0.7f ? sf::Color(34 * (moisture - 0.6f) / 0.1f, 160 - 60 * (moisture - 0.6f) / 0.1f,
...
sf::Color getHighlandsTerrainColor(float elevation, float moisture) {
    ...
    sf::Color color =
            ...
            sf::Color(112 + 110 * (moisture - 0.6f) / 0.4f, 128 + 56 * (moisture - 0.6f) / 0.4f,
...
sf::Color getTerrainColor(float elevation, float moisture) {
    sf::Color color =
            ...
            elevation < 0.14f ? sf::Color(std::pow((elevation - 0.11f) / 0.03f, 0.3f) * 48.f,
...
void processWorkItem(std::vector<sf::Vertex> &vertices, const WorkItem &workItem) {
    ...
    for (unsigned int y = rowStart; y < rowEnd; y++) {
        for (int x = 0; x < resolutionX; x++) {
            ...
            if (x > 0) {
                ...
                vertices[arrayIndexBase + 0].texCoords = computeNormal(x, y, getElevation(x - 1, y),
                                                                       getElevation(x + 1, y), getElevation(x, y + 1),
                                                                       getElevation(x, y - 1));
            }
            ...
            if (x > 0) {
                ...
                vertices[arrayIndexBase + 1].texCoords = computeNormal(x, y + 1, getElevation(x - 1, y + 1),
            ...
            vertices[arrayIndexBase + 2].texCoords = computeNormal(x + 1, y + 1, getElevation(x, y + 1),
            ...
            if (y > rowStart) {
                ...
                vertices[arrayIndexBase + 5].texCoords = computeNormal(x + 1, y, getElevation(x, y),
                                                                       getElevation(x + 2, y),
                                                                       getElevation(x + 1, y + 1),
                                                                       getElevation(x + 1, y - 1));
            }
    ...
    std::memcpy(workItem.targetBuffer + (resolutionX * rowStart * 6), &vertices[0],
