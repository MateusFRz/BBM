...
void test(){
    ...
    Beer * b= new Beer("okoko",10,10,20,"bellebrune","ko technique");
    addBeer(b);

    Ingredient * i =new Ingredient("malt","cereal");
    addIngredient(i);
    Ingredient * o =new Ingredient("houblon","cereal");
    addIngredient(o);

    Recipe * r = new Recipe(1,27);
    r->addIngredient(i);
    r->addIngredient(o);
    addRecipe(r);
...
void test2(Register *reg){

    Beer * b= new Beer("corona",10,10,20,"bellebrune","virus");
    Beer * p= new Beer("86",15,15,50,"verte","biere");

    reg->addBeer(b);
    reg->addBeer(p);

    Ingredient * i =new Ingredient("fraise","fruit");
    Ingredient * o =new Ingredient("cerise","fruit");

    reg->addIngredient(i);
    reg->addIngredient(o);


    Recipe * r = new Recipe(2,27);
    r->addIngredient(i);
    r->addIngredient(o);

    vector<Ingredient*>liste = reg->getListIngredient();
    Recipe * f = new Recipe(3,15);
    r->addIngredient(liste[1]);
    r->addIngredient(o);
...
void testage(){
    QTextStream(stdout) << "lancement Persistance : " << endl;

    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    QString path = QDir::currentPath();
    db.setDatabaseName(path+"/DataBase_Bar.sqlite");

    if (!db.open()) {
        QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
        QString path = QDir::currentPath();
        db.setDatabaseName(path+"/DataBase_Bar.sqlite");
        QTextStream(stdout) << "creation d'une nouvelle DataBase" << endl;
        db.open();
        deleteAllTable();
        createAllTable();
    }
    else{
        QTextStream(stdout) << "Ouverture de la DataBase existante" << endl;
    }
  ...
  // test();
    ...
    db.close();
    QTextStream(stdout) << "Fin Persistance" << endl;

        // chercher : dans l'aide : writable pour le path de la DataBase
}
