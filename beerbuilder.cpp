...
BeerBuilder::BeerBuilder(QObject *parent) : QObject(parent)
{
...
Beer * BeerBuilder::creatBeer( QString hopIng,  QString maltyIng, int preparationTime, QString name)
{
    ...
    Beer *beer = new Beer(name,alcohole,IBU,density,color,type,recipe);
    ...
    qDebug()<< beer->name()
            ...
            << beer->density()
            << beer->color()
            << beer->recipe()->getListIngredient().at(0)->name();
